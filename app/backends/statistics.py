...
async def get_statistics(db):
    return Statistic(
        ...
        user_count=(
            await db.execute(select(func.count()).select_from(select(User).subquery()))
        ).scalar_one(),
        enrolled_user_count=(
            await db.execute(
                select(func.count()).select_from(
                    select(User).filter(User.is_dimigo == True).subquery()
                )
            )
        ).scalar_one(),
        graduated_user_count=(
            await db.execute(
                select(func.count()).select_from(
                    select(User)
                    .filter(User.is_dimigo == True)
                    .filter(
                        User.is_dimigo_updated
        ...
        official_service_count=(
            await db.execute(
                select(func.count()).select_from(
                    select(Service).filter(Service.is_official == True).subquery()
                )
            )
        ).scalar_one(),
        unofficial_service_count=(
            await db.execute(
                select(func.count()).select_from(
                    select(Service).filter(Service.is_official == False).subquery()
                )
            )
        ).scalar_one(),
        login_count=0,
...
async def update_statistics():
    ...
    async with get_async_session_ctx() as db:
        statics = await db.execute(
            select(Statistic).filter(Statistic.date == datetime.date.today())
        )
